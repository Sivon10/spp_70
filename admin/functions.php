...
function ubah_siswaku($data)
{
	...
	$query = "UPDATE tb_siswaku SET nis = '$nis',nama ='$nama' , id_kelas='$id_kelas',alamat='$alamat',no_telp='$no_telp',id_spp='$id_spp' where nisn='$id' ";
...
function tambah_siswa($data)
{
	...
	$no_telp = htmlspecialchars($data["no_telp"]);
	$id_spp = htmlspecialchars($data["id_spp"]);
	$cek_nisn = mysqli_query($conn, "SELECT nisn FROM tb_siswaku WHERE nisn = $nisn");
	if (mysqli_fetch_array($cek_nisn)) {
		return false;
		echo "
		<script>
	...
	$query = "INSERT INTO tb_siswaku VALUES('$nisn', '$nis', '$nama',  '$alamat', '$no_telp', '$id_spp','$id_kelas')";
	mysqli_query($conn, $query);
	return mysqli_affected_rows($conn);
}
...
{
	global $conn;
	mysqli_query($conn, "DELETE FROM tb_siswaku WHERE nisn = $id");
	return mysqli_affected_rows($conn);
}
...
{
	$query = "SELECT * FROM tb_siswaku JOIN tb_kelas ON tb_siswaku.id_kelas = tb_kelas.id_kelas JOIN tb_spp ON tb_siswaku.id_spp = tb_spp.id_spp WHERE
	
		nisn LIKE '%$kataKunci%' OR
		nis LIKE '%$kataKunci%' OR
		nama LIKE '%$kataKunci%' OR
		alamat LIKE '%$kataKunci%' OR
		no_telp LIKE '%$kataKunci%'";
